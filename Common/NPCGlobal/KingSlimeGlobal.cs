...
namespace EnhanceBoss.Common.NPCGlobal;
public class KingSlimeGlobal : GlobalNPC {
    public override bool AppliesToEntity(NPC entity, bool lateInstantiation) {
        if(DifficultyManager.CurrentDifficulty == DifficultyLevel.Normal)
            return false;
        return entity.netID == 50 || entity.aiStyle == NPCAIStyleID.KingSlime;
    }
        ...
        if(!Main.player[npc.target].dead && npc.timeLeft > 10 && npc.ai[2] >= 300f && npc.ai[1] < 5f && npc.velocity.Y == 0f) {
            ...
            if(Main.netMode != NetmodeID.MultiplayerClient) {
                ...
                while(!flag9 && num243 < 100) {
                    ...
                    if(num245 >= point4.Y - num242 && num245 <= point4.Y + num242 && num244 >= point4.X - num242 && num244 <= point4.X + num242 || num245 >= point3.Y - num241 && num245 <= point3.Y + num241 && num244 >= point3.X - num241 && num244 <= point3.X + num241 || Main.tile[num244, num245].HasTile)
                        continue;
                    int num246 = num245;
                    int num247 = 0;
                    if(Main.tile[num244, num246].HasTile && Main.tileSolid[Main.tile[num244, num246].TileType] && !Main.tileSolidTop[Main.tile[num244, num246].TileType])
                        num247 = 1;
                    else {
                        for(; num247 < 150 && num246 + num247 < Main.maxTilesY; num247++) {
                            int num248 = num246 + num247;
                            if(Main.tile[num244, num248].HasTile && Main.tileSolid[Main.tile[num244, num248].TileType] && !Main.tileSolidTop[Main.tile[num244, num248].TileType]) {
                                num247--;
                                break;
                            }
                        }
            ...
            if(!flag7) {
                ...
                if(npc.ai[0] >= 0f) {
                    ...
                    if(npc.ai[1] == 3f) {
                        ...
                        npc.velocity.Y = -6f;
                        npc.velocity.X += 5.5f * npc.direction;
                        npc.ai[0] = -60f;
                        npc.ai[1] += 1f;
                    }
